...
class InstagramScraper(object):
    ...
    def __init__(self, **kwargs):
        default_attr = dict(username='', usernames=[], filename=None,
                            ...
                            verbose=0, include_location=False, filter=None, proxies={}, no_check_certificate=False,
                                                        template='{urlname}', log_destination='', idf=0)
        ...
        try:
            for value in self.usernames:
                ...
                for item in tqdm.tqdm(media_generator(value), desc='Searching {0} for posts'.format(value), unit=" media",
                    ...
                    details = self.__get_media_details(item['shortcode'])
                    username = details['owner']['username']
                    ...
                    item['followers'] = user_info['edge_followed_by']['count']
    ...
    def save_json(data, dst='./'):
        ...
        for point in data['GraphImages'].copy():
            for i in point.copy():
                if i != "edge_liked_by" and i != "edge_media_to_comment" and i != "followers":
                    del point[i]
